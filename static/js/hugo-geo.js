...
$(document).ready(function () {
	...
	d3.json(window.baseURL + "/js/world.json", function (error, world) {
		...
		var projection = d3.geo.orthographic().scale(1000).translate([250, height]).rotate([-80]).clipAngle(90).precision(0.1);

		var path = d3.geo.path().projection(projection);

		var graticule = d3.geo.graticule();

		var svg = d3.select("#menu").insert("svg", "#menu-content").attr("id", "menu-globe").attr("width", width).attr("height", height);
